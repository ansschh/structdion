...
class Trainer(torch.distributed.checkpoint.stateful.Stateful, Configurable):
    ...
    def train_step(
        ...
        if hasattr(self.optimizers, "get_optimizer_metrics"):
            extra_metrics.update(self.optimizers.get_optimizer_metrics())
    ...
    def should_continue_training(self) -> bool:
        if self.step >= self.config.training.steps:
            return False
        time_limit = self.config.training.time_limit_s
        if time_limit > 0 and time.perf_counter() - self._train_start_time >= time_limit:
            logger.info(f"Time limit ({time_limit}s) reached at step {self.step}")
            return False
        return True
